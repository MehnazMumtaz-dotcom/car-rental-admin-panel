...
function StatCard({ icon: Icon, label, value, sub, tone }) {
  ...
  return (
    <div className="bg-surface rounded-xl shadow-card border border-borderColor p-3 sm:p-4 flex items-center gap-2 sm:gap-3 min-w-0">
      <div className={`w-9 h-9 sm:w-11 sm:h-11 rounded-full flex items-center justify-center shrink-0 ${tones[tone]}`}>
        <Icon size={18} />
      </div>

      <div className="min-w-0">
        <p className="text-xs text-textSecondary truncate">{label}</p>
        <p className="text-lg sm:text-2xl font-semibold text-textPrimary">
          {value}
        </p>
        <p className={`text-xs font-medium truncate ${tones[tone].split(" ")[1]}`}>
          {sub}
        </p>
      </div>
    </div>
...
export default function SLATimers() {
  ...
  const fetchComplaints = useSLAStore((s) => s.fetchComplaints);
  ...
  const stats = useMemo(() => {
    ...
    const statuses = active.map((c) =>
      getComplaintStatus(c, now).status
    );
    ...
    const breached = statuses.filter(
      (s) => s === "breached"
    ).length;
    ...
    <div className="min-h-screen bg-background p-3 sm:p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 text-sm text-textPrimary">
          SLA Timers are connected with backend complaints API.
          Breached complaints are auto-escalated.
        </div>
